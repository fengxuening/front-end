  ...
  <div class="MainDiv" style="height: 845px">
    ...
    <a-menu  v-model:selectedKeys="tabskey" mode="inline" style="border: #5db4f3 2px solid;height: 100%" @click="menuChange">
      <a-menu-item key="1" v-if="showTabs.showList[0]===1">
        <span style="font-weight: bold;">{{menuNames[1].children[0].menuName}}</span>
      </a-menu-item>
      <a-menu-item key="2" v-if="showTabs.showList[1]===1">

        <span style="font-weight: bold;">{{menuNames[1].children[1].menuName}}</span>
      </a-menu-item>
      <a-menu-item key="3" v-if="showTabs.showList[2]===1">

        <span style="font-weight: bold;">{{menuNames[1].children[2].menuName}}</span>
      </a-menu-item>
    </a-menu>
    ...
    menuNamesGet(){
      GetMenuTree().then(
          response => {
            ...
            this.menuNames = cc
            this.MenuTitle = cc[1].menuName.split(" ")[0]
          }
      )
    },
    menuChange(item){
      var key = parseInt(item.key)
      var menuName = this.menuNames[1].children[key-1].menuName
      var menu = {menuName:menuName,key:key-1,parentMenuId:94}
